...
\label{subsec:Crowd Control}
	...
	At the discretion of the Sweepstakes Chairman, the responsibility of obtaining, storing, and placing crowd control barriers in position for Sweepstakes races may be delegated to the same organization responsible for placing warning signs in position for the Sweepstakes races. The organization charged with this responsibility may not have to provide sweepers, barricaders, or course marshals for each day of Sweepstakes racing, at the discretion of the Sweepstakes Chairman.
...
\subsubsection{Lead Car}
	...
	The lead car should stay several hundred feet in front of the leading buggy at all times so as not to interfere with that buggy during the race. The following individuals may occupy the Lead Car:
...
\subsubsection{Head Judge}
	...
	The Head Judge shall render the final decisions concerning all protests and appeals based on any information that they have gathered relative to those protests and appeals, and on their interpretation of these Rules, Regulations, and Procedures.
...
\subsubsection{Seeding}
	...
	\begin{enumerate}
		...
		\item The order of seeding shall be based on the weighted average of an entry’s last three years finishing times in the preliminary races, with the time from three years ago being multiplied by one, the time from two years ago being multiplied by two, and the time from last year being multiplied by three. These three numbers would be added together and then divided by six to determine that entry’s seeding time. For example, for race day 2002 the entry’s race day 2001 preliminary time would be multiplied by three and added to the entry’s race day 2000 preliminary time multiplied by two, and then finally added to the entry’s race day 1999 preliminary time. This total would then be divided by six to obtain that entry’s seeding time. Heat and lane selection will proceed as described below.

        \item Time penalties incurred during preliminary races will not be included in calculations to determine an entry’s seeding time in future races.
...
\subsubsection{Heat Selection}
	...
	Representatives of each of the entries shall select the heats that their entries shall compete in using the following procedure:

	\begin{enumerate}

		\item
		The highest seeded entry shall select the heat that it wants to compete in during the preliminary races.

		\item
		The next highest seeded entry shall select the heat that it wants to compete in during the preliminary races, with the provision that it cannot select a heat which already has one or more entries in it, unless all of those other entries are consulted and Sweepstakes grants it permission to do so.

		\item
		The preceding step shall be repeated until all of the seeded entries have selected heats.
...
\subsubsection{Heat Assignments}
	...
	Heat assignments for each class of competition shall be made in the same manner as preliminary heat selection. The purpose of the heat assignments is to help ensure that each heat of the finals races has entries that are as far apart with respect to probable finishing times as is possible, in order to reduce the chance of an accident during any of the finals heats. Heat assignments shall be completed as soon as is practical after the preliminary races have ended.
...
\subsubsection{Rankings}
	...
	After the preliminary races are finished, before any rerun races have taken place, and after all time penalties have been assessed, all of the entries that finished a preliminary race and were not disqualified or granted a rerun, shall be ranked according to their finishing times in those preliminary races for each class of competition.
    The entry with the fastest time shall be ranked first, the entry with the second fastest time shall be ranked second, and so on to the entry with the slowest time, which shall be ranked last.

	The ten highest ranked entries from the women's preliminary races shall be eligible to compete in the women's finals races.
...
Drivers must successfully satisfy all of the following requirements:
	...
	\begin{itemize}
		...
		\item 8 roll maximum transfer from fall. If a driver gets 15 rolls or greater in the fall semester, they may transfer 9 rolls from the fall. If a driver gets 22 rolls or greater in the fall semester, they may transfer 10 rolls from the fall.

		\item Complete their spring semester rolls requirements across a minimum of two days, one of which must not be during Truck Weekend.
...
\subsubsection{Buggy Driver Combination Requirements}
	...
	\begin{itemize}
		...
		\item 5 spring rolls for a driver that did not race in a Sweepstakes race the previous year, with at least one roll using each buggy configuration occurring in the Sweepstakes races. 
		\item 3 spring rolls for a driver that did race in a Sweepstakes race the previous year, with at least one roll using each buggy configuration occurring in the Sweepstakes races.
...
\subsubsection{Buggy Requirements}
	...
	Each buggy must successfully complete a safety inspection in every race configuration, in addition to satisfying all of the prior requirements.
...
\subsubsection{Time Penalties}
    ...
    Each time penalty assigned to an entry will add six (6) seconds to the entry’s final course time.

    If a single entry incurs three (3) or more time penalties in a single heat for separate offenses, that entry will instead be disqualified.
...
\subsubsection{Buggies}
	...
	The combined weight of a buggy and its driver may not change while that buggy is competing in any Sweepstakes race and will result in an entry's disqualification. Weight loss during a race shall be permitted only if the Head Judge rules that the weight loss was unintentional, the weight loss was not caused by a design failure of the buggy, and the weight loss did not interfere with any of the other entries in that heat.
	...
	Any device that allows the driver in the buggy communication with individuals outside of the buggy and provides information to a driver that otherwise could not be known by the driver are illegal to use on Raceday rolls or when performing a pass test. These devices may be used during freeroll and push practice; however, use of these prohibited devices on raceday or when more than one buggy is rolling at the same time will result in immediate disqualification. Violations occuring during Raceday rolls will result in an that entry's disqualification. Examples of the devices are headsets, walkie-talkies, telemetry systems, and video display units.
...
If a violation is committed by an individual affiliated with an organization with an entry in the heat:
    \begin{itemize}
    \item If interference with another entry in the heat occurs, the entry of the offender’s organization shall be disqualified and the other entries may be granted a rerun.
    \item If the offender is within 5 feet of any buggy when the starter signals “READY” in the countdown, the entry of the offender’s organization shall be disqualified.
    \item If the offender is more than 5 feet away from all buggies when the starter signals “READY” and there is no interference with another entry, the entry shall receive a 6 second penalty.
    \end{itemize}

If a violation is committed by an individual that is not affiliated with any of the organizations with entries in the heat:
\begin{itemize}
    \item If interference with any entry in the heat occurs, all entries in the heat may be granted reruns.
    \item If no interference occurs, the results of the heat will stand.
    \item If the offender is a member of a racing organization with no entries in the heat, the organization of the offender may be issued an unsportsmanlike conduct penalty at the discretion of the head judge.
\end{itemize}



	If the nose of an entry’s buggy in any heat is beyond the starting line or if the buggy/pusher is making forward progress when the Starter signals the start of the heat, that entry shall be considered to have false started. 

    If an entry in a heat false starts, the entry shall receive a time penalty equal to the amount of time elapsed between when the buggy first began to make forward progress and when the starter signals the start of the heat, plus an additional time penalty. In cases where the false start is deemed by the head judge to be intentional or excessive, the entry shall be disqualified.

    If an entry in a heat commits a false start, the other entries in the heat may be granted a reroll at the discretion of the head judge.

    If multiple entries in a heat commit a false start, the entry who committed the violation first will be penalized, and all other entries may be granted a reroll at the discretion of the head judge. If an entry false starts after another entry in the heat false starts, they are entitled to keep their time from that heat, plus the amount of time the entry false started by.
...
\subsubsection{Lanes}
	...
	If one full wheel, but not all of an entry’s buggy is out of its assigned lane at any point between the starting line and the end of that lane, the entry may receive one of two penalties:
    \begin{itemize}
    \item If there is not interference with another buggy or pusher, that entry shall receive a 6 second penalty
    \item If there is interference with another buggy or pusher, that entry shall be disqualified
    \end{itemize}
    An entry can receive a maximum of ONE time penalty for lane violations.
    If multiple wheels of an entry’s buggy are out of its assigned lane at any point between the starting line and the end of that lane, that entry shall be disqualified.
...
\subsubsection{Pushing}
    ...
    If a violation occurs where the pusher makes contact with the buggy and has one foot located outside of their legal zone, the entry will receive a time penalty. 

    If a violation occurs where the pusher makes contact with the buggy and has both feet located outside of their legal zone, or if the hill 5 pusher is not in contact with that entry’s buggy when its nose crosses the finish line, the entry will be disqualified. 


	No individual affiliated with an entry’s organization may cause anyone or anything to pace a pusher of any of that organization’s entries while that pusher is competing in a race. Pacing shall be defined as: having a person or device in front or alongside a buggy moving in the same direction for any portion of the race. The next pusher in sequence will not be considered a pacer.

    Entries which violate this rule shall receive a time penalty for each separate instance of pacing. For example, if two separate individuals commit a violation, then the entry would receive two time penalties. Similarly, if one individual paces across two segments of the course (ex. H1, H1-2 transition, freeroll, etc.), then the entry would receive two time penalties.
...
%\label{subsubsec:Driving}
	...
	Each entry's buggy in each heat may not bump into or otherwise contact any other entry's buggy in that same heat at any time between the start and finish of that heat and may result in a disqualification.
	...
	If during a heat, an entry’s driver stops that entry’s buggy, that entry shall be eligible for a rerun provided that one of the following conditions are met:

		\begin{itemize}

			\item The Head Judge determines that an accident was about to take place and that the accident that was about to occur was not due to any failure or foul on the part of the buggy that stopped.

			\item The driver stopped due to a safety concern that was not due to a rules violation or extreme negligence on the part of that driver or that entry's organization.

			\item The driver stopped due to a stop flag. The stop flag must have been given due to a safety concern lower down on the course or due to instruction over the official raceday radio communications.
...
\subsubsection{Tape Review}
	...
	Upon the conclusion of a day of Sweepstakes racing, the Sweepstakes Chairman shall acquire the following video footage for tape review:
...
\subsubsection{Final Decision}
	...
	\begin{itemize}
		...
		\item
		If the protest is allowed, the entry which caused the interference or foul will be penalized as defined in Section \ref{Rules}
